...
import { FaSearch, FaHeart, FaShoppingCart, FaMapMarkedAlt, FaMapMarkerAlt } from 'react-icons/fa'
    ...
    return (
        <>
            <div className='w-full px-16 py-2 bg-themeyellow lg:flex hidden justify-between items-center gap-6 '>
                <h1 className='text-sm font-semibold justify-center items-center gap-2 flex'><FaPhoneVolume className='size-[18px] ' /> <span>+91 890 9841</span></h1>
                <h1 className='text-sm flex font-semibold justify-center items-center gap-2'><FaMapMarkerAlt className='size-[18px] ' /> <span>6346 NYC, Manhattan St 12.</span></h1>
                <h1 className='text-sm flex font-semibold justify-center items-center gap-2'><MdEmail className='size-[18px] ' /> <span>electricashop@company.com</span></h1>
            </div>
            ...
            <nav className='w-full bg-gray-100 flex justify-between items-center gap0-1 lg:px-16 px-6 py-5 sticky top-0 z-50'>
                ...
                <div className={`${isMenuOpen ? 'flex' : 'hidden'} w-full bg-themepurple p-4 absolute top-[80px] left-0`} onClick={closeMenu}>
                    <ul className='flex flex-col justify-center items-center gap-2 w-full'>
                        {navItems.map(({ link, path }) => (
                            <Link key={path} className='text-white uppercase font-semibold cursor-pointer p-3 rounded-lg hover:bg-themeyellow hover:text-black w-full text-center' to={path} span={true} offset={-100} smooth={true}>
                                {link}
                            </Link>
                        ))}
                    </ul>
